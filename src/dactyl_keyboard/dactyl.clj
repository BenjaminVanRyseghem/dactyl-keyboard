...
(def nrows 6)
(def ncols 6)
(def half-connector :xlr) ; :usbc, :rj9 (default) or :xlr

(def full-last-row true)  ; when true, the last row will be fully populated
(def large-pinky true)    ; when true, the (last-row, last column) key will be doubled
(def full-baseplate true) ; when true, the baseplate is full

(def α (/ π 12))          ; curvature of the columns
(def β (/ π 36))          ; curvature of the rows
(def centerrow (- nrows 3)) ; controls front-back tilt
(def centercol 4)           ; controls left-right tilt / tenting
(def tenting-angle (/ π 24)) ; tenting control
(def column-style
  (if (> nrows 5) :orthographic :standard)) ; :standard, :orthographic, or :fixed
...
(def last-full-row (if full-last-row lastrow cornerrow))
...
(defn single-plate
  ([] (single-plate false))
  ([no-holes]
   (let [top-wall   (->> (cube (+ keyswitch-width 3)
                               (if no-holes 14 1.5)
                               plate-thickness)
                         (translate [0
                                     (+ (/ (if no-holes -10 1.5) 2)
                                        (/ keyswitch-height 2))
                                     (/ plate-thickness 2)]))
...
(defn double-plate
  ...
  ([no-holes]
   (let [top-wall   (->> (cube (+ keyswitch-width 3)
                               (if no-holes 25
                                   (+ 2 web-thickness (/ mount-height 2)))
                               plate-thickness)
                         (translate [0
                                     (if no-holes 10
                                         (+ 1 (/ web-thickness 2)
         ...
         side-nub   (->> (binding [*fn* 30] (cylinder 1 2.75))
                         ...
                         (hull (->> (cube 1.5 2.75 plate-thickness)
                                    (translate [(+ (/ 1.5 2) (/ keyswitch-width 2))
                                                ...
                                                (/ plate-thickness 2)]))))
         plate-half (union top-wall left-wall (with-fn 100 side-nub))]
     (translate [0 (- (/ mount-height -2) (/ web-thickness 2) 0.5) 0]
       (union plate-half
              (->> plate-half
                   (mirror [1 0 0])
                   (mirror [0 1 0])))))))
...
(defn apply-key-geometry [translate-fn rotate-x-fn rotate-y-fn column row shape]
  (let [column-angle        (* β (- centercol column))
        ...
        placed-shape-ortho  (->> shape
                                 ...
                                 (translate-fn [0 0 row-radius])
                                 (rotate-y-fn column-angle)
                                 (translate-fn [(- (* (- column centercol) column-x-delta))
                                                0
                                                column-z-delta])
                                 (translate-fn (column-offset column)))
...
(defn apply-double-key-geometry [translate-fn rotate-x-fn rotate-y-fn column row shape]
  (let [column-angle        (* β (- centercol column))
        placed-shape        (->> shape
                                 (translate-fn [0 0 (- row-radius)])
                                 (rotate-x-fn (* α (- centerrow row)))
        ...
        column-z-delta      (* column-radius (- 1 (Math/cos column-angle)))
        placed-shape-ortho  (->> shape
                                 (translate-fn [0 (- mount-height (+ 1 web-thickness)) (- row-radius)])
                                 (rotate-x-fn (* α (- centerrow (+ 0.5 row))))
                                 (translate-fn [0 0 (+ row-radius (/ web-thickness 2))])
                                 (rotate-y-fn column-angle)
                                 (translate-fn [(- (* (- column centercol) column-x-delta))
                                                0
                                                column-z-delta])
...
(defn double-key-place [column row shape]
  (apply-double-key-geometry translate
                             (fn [angle obj] (rotate angle [1 0 0] obj))
                             (fn [angle obj] (rotate angle [0 1 0] obj))
                             column row shape))
...
(defn double-key-holes
  ([] (double-key-holes false))
  ([no-holes]
   (apply union
          (for [column columns
                row    (range 0 (dec last-full-row))
                :when  (or (.contains [2 3] column)
                           full-last-row
                           (not= row lastrow))]
            (->> (single-plate no-holes)
                 (key-place column row)))
          [(->> (double-plate no-holes) (double-key-place lastcol (dec last-full-row)))]
          (for [column (range 0 lastcol)
                row    (range (dec last-full-row) nrows)
                :when  (or (.contains [2 3] column)
                           full-last-row
                           (not= row lastrow))]
            (->> (single-plate no-holes)
...
(def double-connectors
  (apply union
         ...
         (concat
          ...
          (for [column (range 0 lastcol)]
            (triangle-hulls
             (key-place (inc column) last-full-row web-post-tl)
             (key-place column last-full-row web-post-tr)
             (key-place (inc column) last-full-row web-post-bl)
             (key-place column last-full-row web-post-br)))
          ;; Column connections
          (for [column (range 0 lastcol)]
            (triangle-hulls
             (key-place column (dec last-full-row) web-post-bl)
             (key-place column (dec last-full-row) web-post-br)
             (key-place column last-full-row web-post-tl)
             (key-place column last-full-row web-post-tr)))
          ;; Diagonal connections
          (for [column (range 0 lastcol)]
            (triangle-hulls
             (key-place column (dec last-full-row) web-post-br)
             (key-place column last-full-row web-post-tr)
             (key-place (inc column) (dec last-full-row) web-post-bl)
             (key-place (inc column) last-full-row web-post-tl))))))
...
(def thumborigin
  (map + (key-position 1 last-full-row [(/ mount-width 2) (- (/ mount-height 2)) 0])
       thumb-offsets))
...
(defn larger-plate
  ([] (larger-plate false))
  ([no-holes]
   (let [plate-height (if no-holes 8 (/ (- sa-double-length mount-height) 3))
...
(defn thumb
  ...
  ([no-holes]
   (union
    (thumb-1x-layout (single-plate no-holes))
    (thumb-15x-layout (single-plate no-holes))
    (thumb-15x-layout (larger-plate no-holes)))))
...
(def thumb-connectors
  (union
   ...
   (triangle-hulls ; top two to the main keyboard, starting on the left
    (thumb-tl-place thumb-post-tl)
    (key-place 0 last-full-row web-post-bl)
    (thumb-tl-place thumb-post-tr)
    (key-place 0 last-full-row web-post-br)
    (thumb-tr-place thumb-post-tl)
    (key-place 1 last-full-row web-post-bl)
    (thumb-tr-place thumb-post-tr)
    (key-place 1 last-full-row web-post-br)
    (key-place 2 (inc last-full-row) web-post-tl)
...
(defn case-walls
  ...
  ([no-holes]
   (union
    ...
    (key-wall-brace lastcol last-full-row 0 -1 web-post-br lastcol cornerrow 1 0 web-post-br)
    ; left wall
    ...
    (key-wall-brace 3 lastrow 0.5 -1 web-post-br
                    4 last-full-row (if full-last-row 0 1) -1 web-post-bl)
    (for [x (range 4 ncols)] (key-wall-brace x last-full-row 0 -1 web-post-bl x       last-full-row 0 -1 web-post-br))
    (for [x (range 5 ncols)] (key-wall-brace x last-full-row 0 -1 web-post-bl (dec x) last-full-row 0 -1 web-post-br))
    ; thumb walls
    ...
    (wall-brace thumb-tr-place  0 -1 thumb-post-br
                (partial key-place 3 lastrow) 0 -1 web-post-bl)
    ...
    (hull
     ...
     (key-place 0 last-full-row
                (translate (wall-locate1 (if full-last-row 0 -1) 0) web-post-bl))
     (thumb-tl-place thumb-post-tl))
    (hull
     (thumb-ml-place web-post-tr)
...
(def connector-start
  (map + [3 -3 0]
       (key-position 0 0
                     (map + (wall-locate3 0 1) [0 (/ mount-height 2) 0]))))
(def connector-position [(first connector-start) (second connector-start) 11])

(def rj9-width 14.78)
(def rj9-cube (cube rj9-width 13 22.38))
(def rj9-space (translate connector-position rj9-cube))
(def rj9-holder
  (translate connector-position
             (difference rj9-cube
                         (union (translate [0 2 0] (cube 10.78 9 18.38))
                                (translate [0 0 5] (cube 10.78 13 5))))))

(def xlr-width (+ xlr-offset xlr-diam))
(def xlr-cylinder
  (->> (cylinder (/ xlr-width 2) 9)
       (rotate (/ π 2) [1 0 0])
       (translate [0 2 0])))
(def xlr-space (translate connector-position xlr-cylinder))
(def xlr-holder
  (translate connector-position
             (difference xlr-cylinder
                         (->> (cylinder (/ xlr-diam 2) 9)
                              (rotate (/ π 2) [1 0 0])
                              (translate [0 2 0])))))
...
(defn usb-space
  ([] (usb-space usb-holder-position))
  ([position]
   (->> usb-cube
        (translate [(first position)
                    (second position)
                    (/ (+ (last usb-holder-size) usb-holder-thickness) 2)]))))

(defn usb-holder
  ([] (usb-holder usb-holder-position))
  ([position]
   (translate [(first position)
               (second position)
               (/ (+ (last usb-holder-size) usb-holder-thickness) 2)]
...
(def connector-space
  (case half-connector
    :xlr  xlr-space
    :usbc (usb-space connector-position)
    :rj9  rj9-space
          rj9-space))
...
(def connector-holder
  (case half-connector
    :xlr  xlr-holder
    :usbc (usb-holder connector-position)
    :rj9  rj9-holder
          rj9-holder))
...
(def teensy-holder
  (->> (union
        ...
        (->> (cube teensy-pcb-thickness teensy-holder-length 3)
             (translate [(+ (/ teensy-pcb-thickness 2) 3)
                         teensy-holder-offset
                         (- -1.5 (/ teensy-width 2))]))
        (->> (cube 4 teensy-holder-length 4)
             (translate [(+ teensy-pcb-thickness 5)
                         teensy-holder-offset
                         (- -1 (/ teensy-width 2))]))
...
(defn screw-insert-all-shapes [bottom-radius top-radius height]
  (union
   ...
   (screw-insert 2 (+ lastrow 0.3 (if full-last-row 0.4 0))  bottom-radius top-radius height)
...
(def screw-insert-screw-holes
  (screw-insert-all-shapes 1.7 1.7 350))
...
(defn model-right
  ([main] (model-right main false))
  ([main no-holes]
   (difference
    (union
     (if large-pinky (double-key-holes no-holes) (key-holes no-holes))
     (if large-pinky double-connectors connectors)
     (thumb no-holes)
     thumb-connectors
     (difference (union (case-walls no-holes)
                        screw-insert-outers
                        teensy-holder)
                 (if main (usb-space))
                 connector-space
...
(spit "things/right.scad"
      (write-scad (model-right true)))

(spit "things/left.scad"
      (write-scad (mirror [-1 0 0] (model-right false))))
...
(defn make-full-baseplate [main]
  (let [projection (project (model-right main true))]
    (difference
     (extrude-linear {:height 3} projection)
     (translate [0 0 -10] screw-insert-screw-holes))))

(spit "things/right-plate.scad"
      (write-scad (if full-baseplate (make-full-baseplate true) plate-outline)))

(spit "things/left-plate.scad"
      (write-scad (mirror [-1 0 0] (if full-baseplate (make-full-baseplate false) plate-outline))))

(spit "things/test.scad"
      (write-scad
       (difference (usb-space) (usb-holder))))
